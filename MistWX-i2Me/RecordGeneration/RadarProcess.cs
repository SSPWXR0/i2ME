...
public class RadarProcess
{
    ...
    private static async Task ProcessRadarFrame(Image[] imgs, Image frame, Point<int>[] coords, int ts, TileImageBounds bounds, string dir_path, UdpSender sender, string radar_type, Point<int> OGSize)
    {
        ...
        sender.SendFile(framePath, $"storePriorityImage(FileExtension=.tiff,Location={splitRadarType[1]},ImageType={splitRadarType[0]},IssueTime=\"{DateTimeOffset.FromUnixTimeSeconds(ts).UtcDateTime.ToString("MM/dd/yyyy HH:mm:ss")}\")");
